...
## Task 52 — Growth Metrics Seeding Package: services table + daily shuffle RPC [✅ DONE]
...
## Task 55 — Give seed users their own visible campaigns for the leaderboard: real-celebrity-identity finding, confirmed decision, and mitigation methods for whoever builds this [✅ Done]
...
## Task 66 — Build the listener-facing "listen and earn" UI (unblocked)

**Status:** ✅ Fully specified and actionable.

#### Core Decision Summary

- **Listen-and-Earn is a listener-side feature** (not artist-facing). No campaign-count gate.
- **UI location**: Task board, enrollment, and balance display live on `mavins-web` (`/earn` page). Velune handles playback and 60‑second tracking only (read‑only for campaigns).
- **Identity model**: Fully anonymous – device ID only. Real identity (email, KYC) deferred until withdrawal. No login, no password, no Supabase Auth dependency.
- **Reward model**: Points (like Sweatcoin), not cash. Cash conversion deferred to redemption step.
- **Viewing balance on the web**: Signed token handoff from Velune to browser (HMAC‑signed device ID, short expiry). No account creation needed.
- **Earning flow**:
  1. User browses task board on `mavins-web` → selects a song → web opens Velune with a `reward=true` intent.
  2. Velune plays the song; if played directly from Velune (without the intent), **no reward**.
  3. Velune displays a 60‑second progress pill (like rewarded ads).
  4. After 60 seconds, Velune records the qualifying play in `listener_sessions` (`is_qualifying_play = true`) and updates `listener_earnings` for the device ID.
  5. User **must** return to the web to select the next song – cannot chain plays inside Velune.

#### Implementation Tasks (Priority Order)

1. Build `POST /api/listener/token` – issues HMAC‑signed token for device ID.
2. Build `GET /api/listener/balance?token=...` – verifies token, returns balance.
3. Build the `/earn` page on `mavins-web` – shows task board + balance (with token handoff).
4. Add deep‑link handler in Velune – opens web with token.
5. Add 60‑second progress pill UI in Velune for reward‑eligible plays.
6. Wire the web‑to‑app intent with the `reward=true` flag.
7. Connect the task board to actual campaign data (reward‑eligible songs).

#### Database

- `listener_sessions` and `listener_earnings` tables already exist (migration 030). No new schema changes required.
...
B-Pay-backend becomes the **single payment infrastructure** for all transactions processed by the Mavin ecosystem. Developers and end‑users interact exclusively with B-Pay's unified API and custom‑branded checkout pages. The backend intelligently routes each transaction to the appropriate provider (Korapay, Paystack, Wise, Flutterwave, Lizzysub, Juicyway, TransFi, Dodopay, etc.) based on transaction parameters. All providers are **fully integrated** (real API calls, not stubs) and all webhooks are implemented mirroring the Korapay pattern. The system is operational only when the required API keys are set via environment variables (on Render). Duplicate providers (those overlapping Korapay) are fully integrated and kept on standby for easy primary provider switching.

### Core Principles

1. **All providers are fully integrated** – each adapter implements the complete API of the respective provider (pay‑in, pay‑out, verification, refund, webhooks).
2. **Operational only when keys are set** – the system checks for the presence of each provider's required environment variables; if missing, that provider is disabled gracefully.
3. **All webhooks fully set up** – every provider's webhook endpoints are implemented with signature verification, idempotency, and logging, following the same pattern as Korapay.
4. **Duplicate providers on standby** – providers that overlap with Korapay (Paystack, Flutterwave, etc.) are fully integrated and can be swapped in as primary by toggling a config flag.
5. **Custom checkout pages** – users of B-Pay see a branded B-Pay checkout UI (hosted on `mavins-web` or a dedicated front‑end). The backend creates a checkout session and returns session data; the front‑end renders the appropriate payment method UI (cards, bank transfers, etc.) without revealing the underlying provider.
6. **Documentation portal** – a front‑end (e.g., `/docs`) where developers can read the B-Pay API reference, test endpoints, and understand how to integrate payments, payouts, and subscriptions without ever knowing which provider is used.

### Provider List & Their Specific Roles

| Provider | Core Capabilities | Role / Gap Filled |
|----------|-------------------|-------------------|
| **Korapay** | Pay‑in, pay‑out, bank transfers, mobile money, bill payments (Africa) | Primary default |
| **Paystack** | Cards, bank transfers, USSD, recurring payments | Standby / duplicate |
| **Wise** | International transfers (multi‑currency payouts) | Gap: outside Africa |
| **Remita** | Government payments, taxes, bills | Gap: specific bills |
| **Flutterwave** | Cards, bank transfers, mobile money, crypto (Barter) | Standby / duplicate |
| **Payscribe** | To be assessed; integrated if unique | Gap |
| **Lizzysub** | VTU (airtime, data, utility bills) | Gap: Korapay doesn't offer VTU |
| **Juicyway** | Crypto‑to‑fiat, cross‑border crypto | Gap |
| **TransFi** | Stablecoin conversions, on‑/off‑ramp | Gap |
| **Xixapay** | To be assessed; integrated if unique | Gap |
| **Dodopay** | Digital assets, revenue‑like subscriptions (similar to RevenueCat) | Gap: subscriptions |
| **Presmit** | To be assessed; integrated if unique | Gap |

### Routing Logic (Fixed, Deterministic)

The router uses a **capability registry** (JSON or DB) to determine which provider to use for a given transaction. The registry stores supported `transaction_type`, `currency`, `country`, `payment_method`. If Korapay supports it → route to Korapay. If not, route to the specific gap‑filling provider.

**Examples:**
- `type='payin'`, `currency='NGN'`, `country='NG'` → Korapay.
- `type='payout'`, `currency='USD'`, `country='US'` → Wise.
- `type='vtus'`, `currency='NGN'` → Lizzysub.
- `type='crypto_payout'`, `currency='USDC'` → Juicyway or TransFi.
- `type='subscription'`, `currency='USD'`, `digital_asset=true` → Dodopay.

### Webhook Implementation (All Providers)

- Each provider has its own webhook endpoint: `/webhooks/{provider}/`.
- The handler parses the provider‑specific payload, verifies the signature, extracts the relevant transaction ID, and updates the internal order/transaction status.
- **Idempotency** is enforced (using a unique `webhook_id` or provider‑provided idempotency key).
- All webhook events are logged with full request/response for debugging.
- The webhook endpoints are **secured** (via a shared secret or signature verification) and are accessible to the provider's callback URLs (configured in the provider's dashboard).

### Custom Checkout Flow

1. The caller (front‑end) sends a request to `/api/checkout` with `amount`, `currency`, `payment_method`, `customer_email`, etc.
2. The backend determines the provider (via routing) and calls that provider's API to create a checkout session/order.
3. The backend returns a **unified response** with the checkout session ID, a `redirect_url` (if needed), and any provider‑specific data (e.g., payment intent client secret).
4. The front‑end renders the **B-Pay branded checkout page** (cards, bank transfer details, etc.) using the returned data.
5. The user completes payment on B-Pay's page (the provider's page is **never** shown to the user).
6. The provider sends a webhook to B-Pay; B-Pay updates the order status and can optionally notify the front‑end via polling or WebSocket.

### Implementation Plan (Phased)

1. **Phase 1 – Core Router & Capability Registry**
   - Design the registry (JSON or DB schema).
   - Implement the deterministic router.
   - Write unit tests for routing logic.

2. **Phase 2 – Korapay Adapter (Full Integration)**
   - Implement all Korapay endpoints: pay‑in, pay‑out, bill payment, verification.
   - Implement webhook handler with signature verification.

3. **Phase 3 – All Other Providers (Full Integration)**
   - Implement each adapter following the same pattern.
   - Implement webhook handlers for each.

4. **Phase 4 – Unified Checkout API**
   - Build `/api/checkout` and `/api/checkout/status` endpoints.
   - Build the front‑end checkout UI (on `mavins-web`).

5. **Phase 5 – Documentation Portal**
   - Build `/docs` on `mavins-web` with API reference, guides, and interactive testing (using API keys generated in the B-Pay dashboard).
